...
const log = {
    ...
    section: (msg) => console.log(`${colors.cyan}${'='.repeat(50)}\n${msg}\n${'='.repeat(50)}${colors.reset}`)
};
...
function createZipArchive(version, buildType) {
    const zipName = `wp-imgpressor-${version}${buildType === 'patch' ? '-patch' : ''}.zip`;
    ...
    try {
        ...
        execSync(`cd "${BUILD_DIR}" && zip -r "${zipPath}" . -x "*.DS_Store" "*.git*"`, {
            stdio: 'pipe'
        });
        ...
        execSync(`cd "${tempExtract}" && zip -r "${zipPath}" wp-imgpressor -x "*.DS_Store" "*.git*"`, {
            stdio: 'pipe'
        });
...
📦 Package Details:
   Version: ${newVersion}
   Type: ${buildType === 'release' ? 'Production Release' : buildType.charAt(0).toUpperCase() + buildType.slice(1) + ' Release'}
   Build Dir: ${BUILD_DIR}
   ZIP File: ${zipPath}
